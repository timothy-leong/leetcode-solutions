...
public:
    ListNode *addTwoNumbers(ListNode *l1, ListNode *l2)
    {
         ...
         * a performance penalty. Check with your interviewer if it is ok to override
         * an existing list to save memory.
         */
        function<ListNode *(const ListNode *, const ListNode *, int)> sumLists;
        sumLists = [&](const ListNode *first, const ListNode *second, int carry) -> ListNode *
        {
            ...
            return new ListNode(
                sum % 10,
                sumLists(
                    first ? first->next : nullptr,
                    second ? second->next : nullptr,
                    sum / 10
                )
            );
        };
