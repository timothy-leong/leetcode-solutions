...
class Solution {
 public:
  int totalNQueens(int n) {
    ...
    vector<bool> diagonalOccupied(
        n * 2 -
        1);  // same diagonal means row - col is constant, from -n + 1 to n - 1
    vector<bool> antidiagonalOccupied(
        n * 2 -
        1);  // same antidiag means row + col is constant, from 0 to 2n - 2
