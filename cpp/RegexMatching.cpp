...
public:
    bool isMatch(string s, string p)
    {
         ...
         * | wildcard && isMatch(i, j + 2)                        if i == s.length()                      // The only pattern that matches an empty string is one comprising only asterisks
         ...
         * | dp(i, j + 2) || dp(i + 1, j) || dp(i + 1, j + 2)     if char_match && wildcard               // Either you ignore this match, or you are not the last char to match p[j], or you are the last to match
         */
        ...
        {
            dp[s.length()][j] = j + 1 < p.length() && p[j + 1] == zero_or_more && dp[s.length()][j + 2];
        }
